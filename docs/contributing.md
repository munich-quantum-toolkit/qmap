...
Thank you for your interest in contributing to MQT QMAP! This document outlines
the development guidelines and how to contribute.

We use GitHub to [host code](https://github.com/munich-quantum-toolkit/qmap), to
[track issues and feature requests][issues], as well as accept
[pull requests](https://github.com/munich-quantum-toolkit/qmap/pulls). See
...
- 🐛 Report bugs:
  ...
  <https://github.com/munich-quantum-toolkit/qmap/issues>. Include steps to
...
- 💡 Propose features:
  ...
  <https://github.com/munich-quantum-toolkit/qmap/issues>. Describe the
...
{code}`-DBUILD_MQT_QMAP_TESTS=OFF` to the CMake configure step.
:::

Our CI pipeline on GitHub also collects code coverage information and uploads it
to [Codecov](https://codecov.io/gh/munich-quantum-toolkit/qmap). Our goal is to
...
To build only the Python bindings, pass {code}`-DBUILD_MQT_QMAP_BINDINGS=ON` to
...
in the {code}`python/mqt/qmap` directory. These stub files are auto-generated.
...
MQT QMAP adheres to [Semantic Versioning], with the exception that minor
...
explain how users may need to adapt their usage of MQT QMAP, highlight new
...
When it is time to release a new version of MQT QMAP, create a PR that prepares
...
a breaking change for MQT QMAP. If you are sure that the dependency update does
not introduce any breaking changes for MQT QMAP, you can remove the
...
[Releases page](https://github.com/munich-quantum-toolkit/qmap/releases) on
