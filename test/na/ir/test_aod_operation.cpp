...
TEST(AodOperation, Activate) {
  ...
  const na::AodOperation activate(na::NeutralAtomOpType::AodActivate, {0},
...
TEST(AodOperation, Deactivate) {
  ...
  const na::AodOperation deactivate(na::NeutralAtomOpType::AodDeactivate,
                                    {0, 1}, {na::AodOperation::Dimension::Y},
                                    {0.0}, {1.0});
...
TEST(AodOperation, Move) {
  ...
  const na::AodOperation move(
      na::NeutralAtomOpType::AodMove, {0, 1},
...
TEST(AodOperation, Print) {
  const na::AodOperation activate(na::NeutralAtomOpType::AodActivate, {1},
                                  {na::AodOperation::Dimension::X}, {0.0},
                                  {1.0});
...
TEST(AodOperation, Constructors) {
  ...
  const na::AodOperation move(na::NeutralAtomOpType::AodMove, {0, 1},
                              {dir1, dir2}, {0.0, 1.0}, {1.0, 3.0});
  const na::AodOperation move2("aod_move", {0, 1}, {dir1}, {0.0}, {1.0});
  const na::AodOperation move3(
      na::NeutralAtomOpType::AodMove, {0},
      {std::tuple{na::AodOperation::Dimension::X, 0.0, 1.0}});
  const na::AodOperation::Segment segment(na::AodOperation::Dimension::X, 0.0,
                                          1.0);
  const na::AodOperation move4(na::NeutralAtomOpType::AodMove, {0}, {segment});

  EXPECT_EQ(move.getMaxDistance(na::AodOperation::Dimension::Y), 2.0);
  EXPECT_EQ(move2.getNeutralAtomOpType(), na::NeutralAtomOpType::AodMove);
...
TEST(AodOperation, DefaultConstructionIsUnset) {
  ...
  EXPECT_EQ(operation.getNeutralAtomOpType(), na::NeutralAtomOpType::None);
  EXPECT_EQ(na::getNeutralAtomOpType(operation), std::nullopt);
  EXPECT_FALSE(
      na::hasNeutralAtomOpType(operation, na::NeutralAtomOpType::AodMove));
}

TEST(AodOperation, OverrideMethods) {
  na::AodOperation move(na::NeutralAtomOpType::AodMove, {0},
...
TEST(AodOperation, Invert) {
  ...
  na::AodOperation activate(na::NeutralAtomOpType::AodActivate, {0},
                            {na::AodOperation::Dimension::X}, {0.0}, {1.0});
  activate.invert();
  EXPECT_EQ(activate.getNeutralAtomOpType(),
            na::NeutralAtomOpType::AodDeactivate);
  EXPECT_EQ(activate.getName(), "aod_deactivate");

  na::AodOperation deactivate(na::NeutralAtomOpType::AodDeactivate, {0},
                              {na::AodOperation::Dimension::X}, {0.0}, {1.0});
  deactivate.invert();
  EXPECT_EQ(deactivate.getNeutralAtomOpType(),
            na::NeutralAtomOpType::AodActivate);
