...
"""Framework for evaluating the zoned neutral atom compiler."""
...
if TYPE_CHECKING:
    ...
    from mqt.qmap.na.zoned import RoutingAwareCompiler, RoutingAwareNativeGateCompiler
...
def run_with_process_timeout(func: Callable[P, R], timeout: float, *args: P.args, **kwargs: P.kwargs) -> R:
    ...
    try:
        while True:
            ...
            try:
                status, payload = q.get(timeout=min(remaining, 1.0))
                break
            except queue.Empty:
                if not p.is_alive():
                    msg = f"Child process exited unexpectedly with code {p.exitcode}"
...
def _compile_wrapper(
    compiler: RoutingAwareCompiler | RoutingAwareNativeGateCompiler, qc: QuantumComputation
...
def process_benchmark(
    compiler: RoutingAwareCompiler | RoutingAwareNativeGateCompiler,
    setting_name: str,
    ...
    def _process_cz(self) -> None:
        ...
        for atom, coord in self.atom_locations.items():
            if coord[1] >= y_min:  # atom is in the entanglement zone
                atoms.append(atom)
        if len(atoms) % 2 != 0:
            msg = f"Expected even number of atoms in entanglement zone, got {len(atoms)}"
            raise ValueError(msg)
